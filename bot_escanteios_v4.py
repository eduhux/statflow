...
class _Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path.startswith("/sinais"):
            ...
            # Health check para o Railway saber que o serviço está vivo
...
def telegram(msg):
    try:
        ...
        if r.status_code == 200:
            return True
        log(f"Telegram: {r.json().get('description','')[:80]}", "ERRO")
        return False
    except Exception as e:
...
def ciclo_live():
    ...
    if sinais_live > 0:
        log(f"Ao vivo: {sinais_live} sinal(is).", "SINAL")
...
def teclado():
    while est.rodando:
        try:
            ...
            if cmd == "P":
                est.pausado = not est.pausado
                log("⏸  PAUSADO" if est.pausado else "▶️  RETOMADO", "AVISO")
                telegram(f"{'⏸' if est.pausado else '▶️'} <b>Bot {'pausado' if est.pausado else 'retomado'}.</b> ({hora_br_curta()} Brasília)")
            elif cmd == "S":
                est.rodando = False
                telegram(f"🔴 <b>Bot encerrado.</b> ({hora_br_curta()} Brasília)")
            elif cmd == "R":
                print(est.relatorio())
            elif cmd == "L":
                ...
                if jogos:
                    for j in jogos[:15]:
                        ...
                        print(f"    ⚽ {nc} {gc}x{gf} {nf} ({m}')")
                else:
                    print("    Nenhum jogo ao vivo.")
            else:
                print("  [P] Pausar │ [S] Sair │ [R] Relatório │ [L] Jogos")
...
def main():
    print("""
╔═══════════════════════════════════════════════════════════════╗
║   🤖 BOT DE ESCANTEIOS v4.5 — AGENDA + PRÉ + LIVE + SITE    ║
╠═══════════════════════════════════════════════════════════════╣
║  [P] Pausar  [S] Sair  [R] Relatório  [L] Jogos ao vivo      ║
╠═══════════════════════════════════════════════════════════════╣
...
║  🌐 Site web: abra o index.html no navegador                  ║
╚═══════════════════════════════════════════════════════════════╝
    """)

    log("Iniciando v4.5...", "OK")
    ...
    # ── Inicia servidor web embutido ──
    if _iniciar_servidor_web():
        log(f"🌐 Servidor web ativo em http://localhost:{PORTA_WEB}/sinais", "OK")
    else:
        log(f"⚠️  Porta {PORTA_WEB} ocupada — servidor web não iniciou.", "AVISO")

    msg_inicio = (
        "🤖 <b>Bot de Escanteios v4.5</b>\n"
        "AGENDA + PRÉ-LIVE + AO VIVO + 🧠 IA + 🌐 SITE\n\n"
        f"📡 {len(TODAS_LIGAS)} ligas monitoradas\n"
        f"🧠 Gemini: <b>{'Ativo' if USAR_GEMINI else 'Desativado'}</b>\n"
        f"🌐 Site: <b>abra o index.html no navegador</b>\n\n"
        "<b>Frequência:</b>\n"
        f"📅 Agenda do dia: a cada {CICLO_AGENDA_SEG // 60}min\n"
        f"📋 Pré-live (até 3h antes): a cada {CICLO_PRE_SEG // 60}min\n"
        f"🔴 Ao vivo: a cada {CICLO_LIVE_SEG}s\n\n"
        "<b>Sinais:</b>\n"
        "📅 Agenda · 📋 Over/Under pré-jogo\n"
        "🔴 Over/Under · Momentum · Pressão\n"
        "👑 Domínio · Handicap · Quente · 2T\n"
        "😤 Desespero · Bloqueios · Seca\n"
        "🔔 Over/Under 1º Tempo\n\n"
    ...
    while est.rodando:
        try:
            if not est.pausado:
                ...
                log("Pausado. [P] para retomar.", "AVISO")
