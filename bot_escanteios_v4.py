...
class _Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path.startswith("/sinais"):
        ...
        elif self.path.startswith("/cartoes"):
            body = _cartoes.get_dados().encode("utf-8")
            self.send_response(200)
            self.send_header("Content-Type", "application/json; charset=utf-8")
            self._headers_cors()
            self.send_header("Content-Length", len(body))
            ...
            body = _resultados.get_dados().encode("utf-8")
            self.send_response(200)
            self.send_header("Content-Type", "application/json; charset=utf-8")
            self._headers_cors()
            self.send_header("Content-Length", len(body))
...
# ═══════════════════════════════════════════════════════
#  GERENCIADOR DE SINAIS DE CARTÕES — endpoint /cartoes
# ═══════════════════════════════════════════════════════

class _GerenciadorCartoes:
    """Armazena sinais do bot de cartões em memória e os serve via /cartoes."""
    MAX = 120

    def __init__(self):
        self.sinais = []
        self.lock   = threading.Lock()
        self.stats  = {"total": 0, "quente": 0, "morno": 0, "observar": 0,
                       "ultimo_update": None}
        self._carregar()

    def _carregar(self):
        arq = "sinais_cartoes.json"
        if os.path.exists(arq):
            try:
                with open(arq, "r", encoding="utf-8") as f:
                    d = json.load(f)
    ...
    def _salvar(self):
        try:
            with open("sinais_cartoes.json", "w", encoding="utf-8") as f:
                json.dump({"sinais": self.sinais, "stats": self.stats},
                          f, ensure_ascii=False, indent=2)
        except Exception:
            pass
    ...
    def adicionar(self, sinal: dict):
        with self.lock:
            agora = datetime.now(timezone.utc) - timedelta(hours=3)
            sid   = f"cart_{sinal.get('fixture_id', sinal.get('fid',''))}_{agora.strftime('%Y%m%d_%H%M')}"
            if any(s.get("id") == sid for s in self.sinais):
                return

            tier = str(sinal.get("tier", sinal.get("tier_emoji",""))).replace("🔥","QUENTE").replace("⚡","MORNO").replace("👀","OBSERVAR")
            ...
            registro = {
                ...
                "liga":       sinal.get("liga", ""),
                "pais":       sinal.get("pais", ""),
                "dt":         sinal.get("dt_iso", sinal.get("datetime_iso", "")),
                "lambda_total": sinal.get("lambda", sinal.get("lambda_total", 0)),
                "lambda_casa":  sinal.get("lambda_casa", 0),
                "lambda_fora":  sinal.get("lambda_fora", 0),
                "prob_over35":  sinal.get("prob_over35", sinal.get("p_over35", 0)),
                "prob_over45":  sinal.get("prob_over45", sinal.get("p_over45", 0)),
                "prob_over55":  sinal.get("prob_over55", sinal.get("p_over55", 0)),
                "mercado":      sinal.get("mercado_rec", sinal.get("mercado", "")),
                "odd":          sinal.get("odd_rec", sinal.get("odd", 0)),
                "ev":           sinal.get("ev", 0),
                "stake":        sinal.get("stake", 0),
                ...
                "classico":     sinal.get("classico", False),
                "decisivo":     sinal.get("decisivo", False),
                "arb_nome":     sinal.get("arb_nome", ""),
                "arb_cartoes":  sinal.get("arb_cartoes", 0),
                "analise_ia":   sinal.get("analise_ia", sinal.get("analise_texto", "")),
            }
            ...
            self.stats["ultimo_update"] = agora.isoformat()
            t = tier.upper()
            if "QUENTE" in t: self.stats["quente"] = self.stats.get("quente",0) + 1
            elif "MORNO" in t: self.stats["morno"] = self.stats.get("morno",0) + 1
            else: self.stats["observar"] = self.stats.get("observar",0) + 1
            self._salvar()
...
# ═══════════════════════════════════════════════════════
#  GERENCIADOR DE SINAIS 1X2 — endpoint /resultados
# ═══════════════════════════════════════════════════════

class _GerenciadorResultados:
    """Armazena sinais do bot 1x2 em memória e os serve via /resultados."""
    MAX = 120

    def __init__(self):
        self.sinais = []
        self.lock   = threading.Lock()
        self.stats  = {"total": 0, "elite": 0, "quente": 0, "morno": 0,
                       "ultimo_update": None}
        self._carregar()

    def _carregar(self):
        arq = "sinais_resultados.json"
        if os.path.exists(arq):
            try:
                with open(arq, "r", encoding="utf-8") as f:
                    d = json.load(f)
    ...
    def _salvar(self):
        try:
            with open("sinais_resultados.json", "w", encoding="utf-8") as f:
                json.dump({"sinais": self.sinais, "stats": self.stats},
                          f, ensure_ascii=False, indent=2)
        except Exception:
            pass
    ...
    def adicionar(self, sinal: dict):
        with self.lock:
            agora = datetime.now(timezone.utc) - timedelta(hours=3)
            sid   = f"res_{sinal.get('fid','')}_{sinal.get('mkt',{}).get('chave','')}_{agora.strftime('%Y%m%d_%H%M')}"
            if any(s.get("id") == sid for s in self.sinais):
                return
            ...
            registro = {
                ...
                "pais":       sinal.get("pais", ""),
                "dt":         sinal.get("dt_iso", ""),
                "prob_1":     probs.get("p1", 0),
                "prob_x":     probs.get("px", 0),
                "prob_2":     probs.get("p2", 0),
                "prob_1x":    probs.get("p1x", 0),
                "prob_x2":    probs.get("px2", 0),
                "prob_12":    probs.get("p12", 0),
                "mercado":    mkt.get("nome", ""),
                "odd":        mkt.get("odd", 0),
                "ev":         mkt.get("ev", 0),
                "edge":       mkt.get("edge", 0),
                "stake":      sinal.get("stake", 0),
                ...
                "classico":   sinal.get("classico", False),
                "decisivo":   sinal.get("decisivo", False),
                "lh_casa":    sinal.get("lh", 0),
                "la_fora":    sinal.get("la", 0),
                "hforma":     sinal.get("hforma", ""),
                "aforma":     sinal.get("aforma", ""),
            }
            ...
            self.stats["ultimo_update"] = agora.isoformat()
            t = tier.upper()
            if "ELITE" in t: self.stats["elite"] = self.stats.get("elite",0) + 1
            elif "QUENTE" in t: self.stats["quente"] = self.stats.get("quente",0) + 1
            elif "MORNO" in t: self.stats["morno"] = self.stats.get("morno",0) + 1
            self._salvar()
...
# Instâncias globais
_cartoes    = _GerenciadorCartoes()
_resultados = _GerenciadorResultados()
...
def ciclo_mestre():
    ...
    MODO PRÉ-JOGO APENAS — sinais ao vivo desativados.
    """
    ...
    # 2. Pré-live (a cada 5min) — único ciclo ativo
    try:
        ciclo_pre_live()
    except Exception as e:
        log(f"Erro pré-live: {e}", "ERRO")

    # CICLO AO VIVO DESATIVADO — bot opera somente com sinais pré-jogo
...
╚═══════════════════════════════════════════════════════════════╝
    ...
    # ── Inicia o bot de cartões em thread separada ──
    def _iniciar_bot_cartoes():
        try:
            import importlib.util
            arq = os.path.join(os.path.dirname(__file__), "bot_cartoes_v6.py")
            if not os.path.exists(arq):
                log("bot_cartoes_v6.py não encontrado — bot de cartões não iniciado.", "AVISO")
                return
            spec   = importlib.util.spec_from_file_location("bot_cartoes", arq)
            modulo = importlib.util.module_from_spec(spec)
            token_cartoes = os.environ.get("TELEGRAM_TOKEN_CARTOES", "")
            if token_cartoes:
                modulo.TELEGRAM_TOKEN   = token_cartoes
                modulo.TELEGRAM_CHAT_ID = TELEGRAM_CHAT_ID
            spec.loader.exec_module(modulo)
            modulo._web_registrar_cartao = lambda sinal: _cartoes.adicionar(sinal)
            log("🟨 Bot de Cartões v6 iniciando...", "OK")
            modulo.main()
        except Exception as e:
            log(f"Bot de Cartões — erro ao iniciar: {e}", "ERRO")

    threading.Thread(target=_iniciar_bot_cartoes, daemon=True).start()
    log("🟨 Thread do bot de cartões iniciada.", "OK")

    # ── Inicia o bot de resultado 1X2 em thread separada ──
    def _iniciar_bot_resultado():
        try:
            import importlib.util
            arq = os.path.join(os.path.dirname(__file__), "bot_resultado_1x2_v1.py")
            if not os.path.exists(arq):
                log("bot_resultado_1x2_v1.py não encontrado — bot de resultado não iniciado.", "AVISO")
                return
            spec   = importlib.util.spec_from_file_location("bot_resultado", arq)
            modulo = importlib.util.module_from_spec(spec)
            token_res = os.environ.get("TELEGRAM_TOKEN_RESULTADO", "")
            if token_res:
                modulo.TELEGRAM_TOKEN   = token_res
                modulo.TELEGRAM_CHAT_ID = TELEGRAM_CHAT_ID
            spec.loader.exec_module(modulo)
            modulo._web_registrar_resultado = lambda sinal: _resultados.adicionar(sinal)
            log("⚽ Bot de Resultado 1X2 v1 iniciando...", "OK")
            modulo.main()
        except Exception as e:
            log(f"Bot de Resultado 1X2 — erro ao iniciar: {e}", "ERRO")

    threading.Thread(target=_iniciar_bot_resultado, daemon=True).start()
    log("⚽ Thread do bot de resultado 1X2 iniciada.", "OK")
